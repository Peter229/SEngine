...
impl Player {
    ...
    pub fn new() -> Player {
        ...
        let debug = quad::Quad::new([0.0, 0.0], [1.0, 1.0], "assets/textures/crate.jpg".to_string());
        ...
        Player { pos_x: 50.0, pos_y: 60.0, vel_x: 0.0, vel_y: 0.0, quad, right, left, jump, duck, frame: 0.0, width: 16, height: 24, grounded: false, facing: true, debug }
    }
    ...
    pub fn online_state_buffer(&self) -> [u8; 16] {
        ...
        if self.right {
            ...
            state_buffer[control_settings::Player_State::RIGHT as usize] = 2;
        }
        if self.left {
            state_buffer[control_settings::Player_State::LEFT as usize] = 1;
        }else {
            state_buffer[control_settings::Player_State::LEFT as usize] = 2;
        }
        if self.jump {
            state_buffer[control_settings::Player_State::JUMP as usize] = 1;
        }else {
            state_buffer[control_settings::Player_State::JUMP as usize] = 2;
        }
    ...
    pub fn render(&self, shader: &shader::Program) {
        ...
        let mut model = cgmath::Matrix4::from_translation(cgmath::Vector3::new(self.pos_x.floor() - 8.0, self.pos_y.floor() - 15.0, 0.0));
        ...
        //Floor pos to keep pixel perfect
        model = cgmath::Matrix4::from_translation(cgmath::Vector3::new(self.pos_x.floor() - 24.0, self.pos_y.floor() - 24.0, 0.0));
        shader.set_mat4_cg(&model, ffi::CStr::from_bytes_with_nul(b"model\0").expect("CStr::from_bytes_with_nul failed"));
        if self.facing {
